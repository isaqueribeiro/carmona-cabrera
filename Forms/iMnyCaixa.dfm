object FrmCaixa: TFrmCaixa
  Left = 308
  Top = 80
  Width = 793
  Height = 658
  BorderIcons = [biSystemMenu]
  Caption = 'Pagamentos - Movimento Di'#225'rio'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PgCtrlMain: TcxPageControl
    Left = 8
    Top = 336
    Width = 761
    Height = 273
    ActivePage = TbShtPrincipal
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    ClientRectBottom = 269
    ClientRectLeft = 4
    ClientRectRight = 757
    ClientRectTop = 24
    object TbShtPrincipal: TcxTabSheet
      Caption = 'Itens de Pagamento'
      ImageIndex = 79
      object cxGrid1: TcxGrid
        Left = 8
        Top = 8
        Width = 737
        Height = 233
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object cxGridDBTableView1: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          NavigatorButtons.SaveBookmark.Enabled = False
          NavigatorButtons.GotoBookmark.Enabled = False
          DataController.DataSource = DtSItem
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          OptionsBehavior.FocusCellOnTab = True
          OptionsBehavior.FocusFirstCellOnNewRecord = True
          OptionsBehavior.GoToNextCellOnEnter = True
          OptionsData.Appending = True
          OptionsSelection.InvertSelect = False
          OptionsView.Navigator = True
          OptionsView.NoDataToDisplayInfoText = '<Sem Dados Para Exibi'#231#227'o>'
          OptionsView.ColumnAutoWidth = True
          OptionsView.GridLines = glNone
          OptionsView.GroupByBox = False
          Styles.ContentEven = DtmResource.cxStlLinhaImpar
          Styles.ContentOdd = DtmResource.cxStlLinhaPar
          object cxGridDBTableView1cax_seq: TcxGridDBColumn
            Caption = 'Seq'
            DataBinding.FieldName = 'cax_seq'
            Visible = False
            Width = 25
          end
          object cxGridDBTableView1cax_imp: TcxGridDBColumn
            Caption = 'Imp'
            DataBinding.FieldName = 'cax_imp'
            Width = 36
          end
          object cxGridDBTableView1cax_dce: TcxGridDBColumn
            Caption = 'D/C/E/T'
            DataBinding.FieldName = 'cax_dce'
            PropertiesClassName = 'TcxImageComboBoxProperties'
            Properties.Items = <
              item
                Description = 'D'
                ImageIndex = 0
                Value = 'D'
              end
              item
                Description = 'C'
                Value = 'C'
              end
              item
                Description = 'E'
                Value = 'E'
              end
              item
                Description = 'T'
                Value = 'T'
              end>
            Width = 48
          end
          object cxGridDBTableView1mov_codigo: TcxGridDBColumn
            Caption = 'Mov'
            DataBinding.FieldName = 'mov_codigo'
            Width = 50
          end
          object cxGridDBTableView1mov_item: TcxGridDBColumn
            Caption = 'Item'
            DataBinding.FieldName = 'mov_item'
            Width = 41
          end
          object cxGridDBTableView1cax_favorecido: TcxGridDBColumn
            Caption = 'Favorecido'
            DataBinding.FieldName = 'cax_favorecido'
            PropertiesClassName = 'TcxButtonEditProperties'
            Properties.Buttons = <
              item
                Default = True
                Kind = bkEllipsis
              end>
            Properties.OnButtonClick = cxGridDBTableView1cax_favorecidoPropertiesButtonClick
            Width = 186
          end
          object cxGridDBTableView1cax_historico: TcxGridDBColumn
            Caption = 'Hist'#243'rico'
            DataBinding.FieldName = 'cax_historico'
            Width = 156
          end
          object cxGridDBTableView1ccr_codigo: TcxGridDBColumn
            Caption = 'Conta'
            DataBinding.FieldName = 'ccr_codigo'
            PropertiesClassName = 'TcxLookupComboBoxProperties'
            Properties.KeyFieldNames = 'ccr_codigo'
            Properties.ListColumns = <
              item
                FieldName = 'ccr_nome'
              end>
            Properties.ListOptions.CaseInsensitive = True
            Properties.ListOptions.ShowHeader = False
            Properties.ListSource = DtSConta
            Width = 128
          end
          object cxGridDBTableView1cax_valor: TcxGridDBColumn
            Caption = 'Valor'
            DataBinding.FieldName = 'cax_valor'
            Width = 65
          end
        end
        object cxGridLevel1: TcxGridLevel
          GridView = cxGridDBTableView1
        end
      end
    end
  end
  object cxPageControl1: TcxPageControl
    Left = 8
    Top = 84
    Width = 761
    Height = 245
    ActivePage = cxTabSheet1
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    ClientRectBottom = 241
    ClientRectLeft = 4
    ClientRectRight = 757
    ClientRectTop = 24
    object cxTabSheet1: TcxTabSheet
      Caption = 'Principal'
      ImageIndex = 79
      object Shape1: TShape
        Left = 304
        Top = 16
        Width = 441
        Height = 17
        Brush.Color = 12615680
        Pen.Color = clHighlight
        Pen.Style = psClear
      end
      object Label1: TLabel
        Left = 314
        Top = 17
        Width = 161
        Height = 13
        Caption = 'Saldos das Contas Correntes'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindow
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = True
      end
      object btnNovo: TcxButton
        Left = 8
        Top = 8
        Width = 97
        Height = 57
        Hint = 'Excluir'
        Caption = '&Abrir'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        OnClick = btnNovoClick
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFF00}
        LookAndFeel.Kind = lfOffice11
      end
      object btnEditar: TcxButton
        Left = 8
        Top = 64
        Width = 97
        Height = 57
        Hint = 'Excluir'
        Caption = '&Salvar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        OnClick = btnEditarClick
        Glyph.Data = {
          
          FAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFA00}
        LookAndFeel.Kind = lfOffice11
      end
      object btnEncerrar: TcxButton
        Left = 8
        Top = 152
        Width = 193
        Height = 57
        Hint = 'Excluir'
        Caption = '&Fechamento/Extrato'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        OnClick = btnEncerrarClick
        Glyph.Data = {
          
          FAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFA0000}
        LookAndFeel.Kind = lfOffice11
      end
      object cxLabel4: TcxLabel
        Left = 112
        Top = 40
        Caption = 'Usu'#225'rio:'
        Transparent = True
      end
      object DbEdtUsuario: TcxDBTextEdit
        Left = 160
        Top = 40
        DataBinding.DataField = 'cax_usuario'
        DataBinding.DataSource = DtSMaster
        Properties.ReadOnly = True
        TabOrder = 4
        Width = 129
      end
      object cxLabel1: TcxLabel
        Left = 112
        Top = 16
        Caption = 'C'#243'digo:'
        Transparent = True
      end
      object DbEdtCodigo: TcxDBTextEdit
        Left = 160
        Top = 16
        DataBinding.DataField = 'cax_num'
        DataBinding.DataSource = DtSMaster
        ParentFont = False
        Properties.ReadOnly = True
        Style.Color = 8454143
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -11
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = [fsBold]
        Style.IsFontAssigned = True
        TabOrder = 6
        Width = 129
      end
      object cxLabel2: TcxLabel
        Left = 112
        Top = 64
        Caption = 'Data:'
        Transparent = True
      end
      object cxLabel3: TcxLabel
        Left = 112
        Top = 88
        Caption = 'Hora:'
        Transparent = True
      end
      object DbEdtData: TcxDBDateEdit
        Left = 160
        Top = 64
        DataBinding.DataField = 'cax_data'
        DataBinding.DataSource = DtSMaster
        Properties.ReadOnly = False
        TabOrder = 9
        Width = 129
      end
      object DbEdtHora: TcxDBTimeEdit
        Left = 160
        Top = 88
        DataBinding.DataField = 'cax_hora'
        DataBinding.DataSource = DtSMaster
        Properties.ReadOnly = False
        TabOrder = 10
        Width = 129
      end
      object DbGrd: TcxGrid
        Left = 304
        Top = 32
        Width = 441
        Height = 169
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 12
        object DbGridDBTblVw: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          DataController.DataSource = DtSCaixaConta
          DataController.Summary.DefaultGroupSummaryItems = <>
          DataController.Summary.FooterSummaryItems = <>
          DataController.Summary.SummaryGroups = <>
          OptionsBehavior.FocusCellOnTab = True
          OptionsBehavior.FocusFirstCellOnNewRecord = True
          OptionsBehavior.GoToNextCellOnEnter = True
          OptionsData.Appending = True
          OptionsSelection.InvertSelect = False
          OptionsView.Navigator = True
          OptionsView.NoDataToDisplayInfoText = '<Sem Dados Para Exibi'#231#227'o>'
          OptionsView.ColumnAutoWidth = True
          OptionsView.GridLines = glNone
          OptionsView.GroupByBox = False
          Styles.ContentEven = DtmResource.cxStlLinhaImpar
          Styles.ContentOdd = DtmResource.cxStlLinhaPar
          object DbGridDBTblVwccr_codigo: TcxGridDBColumn
            Caption = 'Conta Corrente'
            DataBinding.FieldName = 'ccr_codigo'
            PropertiesClassName = 'TcxLookupComboBoxProperties'
            Properties.KeyFieldNames = 'ccr_codigo'
            Properties.ListColumns = <
              item
                FieldName = 'ccr_nome'
              end>
            Properties.ListOptions.CaseInsensitive = True
            Properties.ListOptions.ShowHeader = False
            Properties.ListSource = DtSConta
            Width = 256
          end
          object DbGridDBTblVwcax_saldo: TcxGridDBColumn
            Caption = 'Saldo Inicial'
            DataBinding.FieldName = 'cax_saldo'
            Width = 94
          end
          object DbGridDBTblVwcax_tipo: TcxGridDBColumn
            Caption = 'Pag/Rec'
            DataBinding.FieldName = 'cax_tipo'
            PropertiesClassName = 'TcxImageComboBoxProperties'
            Properties.Items = <
              item
                Description = 'Pagadora'
                ImageIndex = 0
                Value = 'P'
              end
              item
                Description = 'Recebedora'
                Value = 'R'
              end>
            Width = 89
          end
        end
        object DbGridLvl: TcxGridLevel
          GridView = DbGridDBTblVw
        end
      end
      object btnConecta: TcxButton
        Left = 200
        Top = 152
        Width = 97
        Height = 57
        Hint = 'Excluir'
        Caption = '&Conecta'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 13
        OnClick = btnConectaClick
        Glyph.Data = {
          
          FAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFA0000}
        LookAndFeel.Kind = lfOffice11
      end
      object cxLabel5: TcxLabel
        Left = 112
        Top = 112
        Caption = 'Unidade:'
        Transparent = True
      end
      object DbLkpCmbBxUNegocio: TcxDBLookupComboBox
        Left = 160
        Top = 112
        DataBinding.DataField = 'uni_codigo'
        DataBinding.DataSource = DtSMaster
        Properties.KeyFieldNames = 'uni_codigo'
        Properties.ListColumns = <
          item
            FieldName = 'uni_nome'
          end>
        Properties.ListOptions.CaseInsensitive = True
        Properties.ListOptions.ShowHeader = False
        Properties.ListSource = DtSUnidade
        Style.Color = 8454143
        TabOrder = 11
        Width = 129
      end
    end
  end
  object PnlMain: TPanel
    Left = 0
    Top = 0
    Width = 777
    Height = 73
    Align = alTop
    TabOrder = 2
    object btnFechar: TcxButton
      Left = 8
      Top = 8
      Width = 97
      Height = 57
      Hint = 'Excluir'
      Caption = '&Fechar'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnClick = btnFecharClick
      Glyph.Data = {
        
        FAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFAFA0000}
      LookAndFeel.Kind = lfOffice11
    end
  end
  object DtSMaster: TDataSource
    DataSet = ClntDtStMaster
    Left = 384
    Top = 224
  end
  object QryMaster: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'numero'
        ParamType = ptInput
        Value = '0'
      end>
    SQL.Strings = (
      'select'
      '  a.`cax_num`,'
      '  a.`cax_data`,'
      '  a.`cax_hora`,'
      '  a.`cax_usuario`,'
      '  a.cax_status,'
      '  a.uni_codigo'
      'from'
      '  `mny_caixa` a'
      'where'
      '  a.cax_num = :numero')
    SQLConnection = FrmLogin.conWebMaster
    Left = 320
    Top = 192
    object QryMastercax_num: TIntegerField
      FieldName = 'cax_num'
      Required = True
    end
    object QryMastercax_data: TDateField
      FieldName = 'cax_data'
    end
    object QryMastercax_hora: TTimeField
      FieldName = 'cax_hora'
    end
    object QryMastercax_usuario: TStringField
      FieldName = 'cax_usuario'
      Size = 60
    end
    object QryMastercax_status: TSmallintField
      FieldName = 'cax_status'
    end
    object QryMasteruni_codigo: TSmallintField
      FieldName = 'uni_codigo'
    end
  end
  object DtStPvdMaster: TDataSetProvider
    DataSet = QryMaster
    Left = 352
    Top = 224
  end
  object ClntDtStMaster: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DtStPvdMaster'
    BeforePost = ClntDtStMasterBeforePost
    OnNewRecord = ClntDtStMasterNewRecord
    Left = 320
    Top = 224
    object ClntDtStMastercax_num: TIntegerField
      FieldName = 'cax_num'
      Required = True
    end
    object ClntDtStMastercax_data: TDateField
      FieldName = 'cax_data'
    end
    object ClntDtStMastercax_hora: TTimeField
      FieldName = 'cax_hora'
    end
    object ClntDtStMastercax_usuario: TStringField
      FieldName = 'cax_usuario'
      Size = 60
    end
    object ClntDtStMastercax_status: TSmallintField
      FieldName = 'cax_status'
    end
    object ClntDtStMasteruni_codigo: TSmallintField
      FieldName = 'uni_codigo'
    end
  end
  object QryMax: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select'
      '  MAX(coalesce(a.`cax_num`,0)) as seq'
      'from'
      '  `mny_caixa` a')
    SQLConnection = FrmLogin.conWebMaster
    Left = 352
    Top = 192
    object QryMaxseq: TBCDField
      FieldName = 'seq'
      Precision = 6
      Size = 0
    end
  end
  object QryTransaction: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'insert into'
      '  sys_usuario'
      '  (usr_codigo, usr_nome)'
      '  values'
      '   (12,'
      '    '#39#39';')
    SQLConnection = FrmLogin.conWebMaster
    Left = 384
    Top = 192
  end
  object QryConta: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select'
      '  a.`ccr_codigo`,'
      
        '  CONCAT(b.`bco_sigla`, " - ", a.`ccr_agencia`, "-", a.`ccr_cont' +
        'a`, "-", a.`ccr_conta_dv`) as ccr_nome'
      'from'
      '  `mny_conta_corrente` a'
      '  join `sys_banco` b on( b.`bco_codigo`=a.`ban_codigo`)'
      'where'
      '  a.`ccr_caixa` = 1'
      'order by 2')
    SQLConnection = FrmLogin.conWebMaster
    Left = 464
    Top = 72
    object QryContaccr_codigo: TSmallintField
      FieldName = 'ccr_codigo'
      Required = True
    end
    object QryContaccr_nome: TStringField
      FieldName = 'ccr_nome'
      Size = 38
    end
  end
  object DtSConta: TDataSource
    DataSet = QryConta
    Left = 496
    Top = 72
  end
  object QryCaixaConta: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftSmallint
        Name = 'cax_num'
        ParamType = ptInput
        Value = 1
      end>
    SQL.Strings = (
      'select'
      '  a.`cax_num`,'
      '  a.`ccr_codigo`,'
      '  a.`cax_saldo`,'
      '  a.`cax_tipo`   '
      'from'
      '  `mny_caixa_conta` a'
      'where'
      '  a.`cax_num` = :cax_num')
    SQLConnection = FrmLogin.conWebMaster
    Left = 640
    Top = 224
    object QryCaixaContacax_num: TIntegerField
      FieldName = 'cax_num'
      Required = True
    end
    object QryCaixaContaccr_codigo: TSmallintField
      FieldName = 'ccr_codigo'
    end
    object QryCaixaContacax_saldo: TFMTBCDField
      FieldName = 'cax_saldo'
      Precision = 17
      Size = 2
    end
    object QryCaixaContacax_tipo: TStringField
      FieldName = 'cax_tipo'
      FixedChar = True
      Size = 1
    end
  end
  object ClntDtStCaixaConta: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DtStPvdCaixaConta'
    BeforePost = ClntDtStCaixaContaBeforePost
    OnNewRecord = ClntDtStCaixaContaNewRecord
    Left = 640
    Top = 256
    object ClntDtStCaixaContacax_num: TIntegerField
      FieldName = 'cax_num'
      Required = True
    end
    object ClntDtStCaixaContaccr_codigo: TSmallintField
      FieldName = 'ccr_codigo'
    end
    object ClntDtStCaixaContacax_saldo: TFMTBCDField
      FieldName = 'cax_saldo'
      DisplayFormat = '#,###,##0.00'
      Precision = 17
      Size = 2
    end
    object ClntDtStCaixaContacax_tipo: TStringField
      FieldName = 'cax_tipo'
      FixedChar = True
      Size = 1
    end
  end
  object DtStPvdCaixaConta: TDataSetProvider
    DataSet = QryCaixaConta
    Left = 672
    Top = 256
  end
  object DtSCaixaConta: TDataSource
    DataSet = ClntDtStCaixaConta
    Left = 672
    Top = 224
  end
  object QryItem: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftSmallint
        Name = 'cax_num'
        ParamType = ptInput
        Value = 1
      end>
    SQL.Strings = (
      'select'
      '  a.`cax_num`,'
      '  a.`cax_seq`,'
      '  a.cax_imp,'
      '  a.`cax_dce`,'
      '  a.`mov_codigo`,'
      '  a.`mov_item`,'
      '  a.`ccr_codigo`,'
      '  a.`cax_favorecido`,'
      '  a.`cax_historico`,'
      '  a.`cax_valor`'
      'from'
      '  `mny_caixa_item` a'
      'where'
      '  a.`cax_num` = :cax_num'
      'order by 1,3')
    SQLConnection = FrmLogin.conWebMaster
    Left = 640
    Top = 352
    object QryItemcax_num: TIntegerField
      FieldName = 'cax_num'
      Required = True
    end
    object QryItemcax_seq: TSmallintField
      FieldName = 'cax_seq'
      Required = True
    end
    object QryItemcax_dce: TStringField
      FieldName = 'cax_dce'
      Size = 1
    end
    object QryItemmov_codigo: TIntegerField
      FieldName = 'mov_codigo'
    end
    object QryItemmov_item: TSmallintField
      FieldName = 'mov_item'
    end
    object QryItemccr_codigo: TSmallintField
      FieldName = 'ccr_codigo'
    end
    object QryItemcax_favorecido: TStringField
      FieldName = 'cax_favorecido'
      Size = 100
    end
    object QryItemcax_historico: TStringField
      FieldName = 'cax_historico'
      Size = 100
    end
    object QryItemcax_valor: TFMTBCDField
      FieldName = 'cax_valor'
      Precision = 17
      Size = 2
    end
    object QryItemcax_imp: TSmallintField
      FieldName = 'cax_imp'
    end
  end
  object DtSItem: TDataSource
    DataSet = ClntDtStItem
    Left = 672
    Top = 352
  end
  object ClntDtStItem: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'DtStPvdItem'
    BeforePost = ClntDtStItemBeforePost
    BeforeDelete = ClntDtStItemBeforeDelete
    OnNewRecord = ClntDtStItemNewRecord
    Left = 640
    Top = 384
    object ClntDtStItemcax_num: TIntegerField
      FieldName = 'cax_num'
      Required = True
    end
    object ClntDtStItemcax_seq: TSmallintField
      FieldName = 'cax_seq'
      Required = True
    end
    object ClntDtStItemcax_imp: TSmallintField
      FieldName = 'cax_imp'
    end
    object ClntDtStItemcax_dce: TStringField
      FieldName = 'cax_dce'
      Size = 1
    end
    object ClntDtStItemmov_codigo: TIntegerField
      FieldName = 'mov_codigo'
    end
    object ClntDtStItemmov_item: TSmallintField
      FieldName = 'mov_item'
      OnValidate = ClntDtStItemmov_itemValidate
    end
    object ClntDtStItemccr_codigo: TSmallintField
      FieldName = 'ccr_codigo'
    end
    object ClntDtStItemcax_favorecido: TStringField
      FieldName = 'cax_favorecido'
      Size = 100
    end
    object ClntDtStItemcax_historico: TStringField
      FieldName = 'cax_historico'
      Size = 100
    end
    object ClntDtStItemcax_valor: TFMTBCDField
      FieldName = 'cax_valor'
      DisplayFormat = '#,###,##0.00'
      Precision = 17
      Size = 2
    end
  end
  object DtStPvdItem: TDataSetProvider
    DataSet = QryItem
    Left = 672
    Top = 384
  end
  object QryItemMax: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftSmallint
        Name = 'cax_num'
        ParamType = ptInput
        Value = 0
      end>
    SQL.Strings = (
      'select'
      '  MAX(coalesce(a.`cax_seq`,0)) as seq'
      'from'
      '  `mny_caixa_item` a'
      'where'
      '  a.cax_num = :cax_num')
    SQLConnection = FrmLogin.conWebMaster
    Left = 640
    Top = 416
    object QryItemMaxseq: TBCDField
      FieldName = 'seq'
      Precision = 6
      Size = 0
    end
  end
  object QryMov: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'mov_codigo'
        ParamType = ptInput
        Value = '0'
      end
      item
        DataType = ftString
        Name = 'mov_item'
        ParamType = ptInput
        Value = '0'
      end>
    SQL.Strings = (
      'select'
      '  a.`mov_codigo`,'
      '  a.`mov_item`,'
      '  c.`pes_razao_social`,'
      '  b.`mov_obs`,'
      '  a.`mov_valor_pagar`'
      'from'
      '  `mny_movimento_item` a'
      '  join `mny_movimento` b on( b.`mov_codigo`=a.`mov_codigo`)'
      '  join `mny_pessoa`    c on( c.`pes_codigo`=b.`pes_codigo`)'
      'where'
      '  a.`mov_codigo`   = :mov_codigo'
      '  and a.`mov_item` = :mov_item')
    SQLConnection = FrmLogin.conWebMaster
    Left = 228
    Top = 376
    object QryMovmov_codigo: TIntegerField
      FieldName = 'mov_codigo'
      Required = True
    end
    object QryMovmov_item: TSmallintField
      FieldName = 'mov_item'
      Required = True
    end
    object QryMovpes_razao_social: TStringField
      FieldName = 'pes_razao_social'
      Size = 200
    end
    object QryMovmov_obs: TStringField
      FieldName = 'mov_obs'
      Size = 200
    end
    object QryMovmov_valor_pagar: TFMTBCDField
      FieldName = 'mov_valor_pagar'
      Precision = 17
      Size = 2
    end
  end
  object TmrConecta: TTimer
    Interval = 30000
    OnTimer = TmrConectaTimer
    Left = 432
    Top = 72
  end
  object QryUnidade: TSQLQuery
    MaxBlobSize = -1
    Params = <>
    SQL.Strings = (
      'select'
      '  a.`uni_codigo`,'
      '  a.`uni_nome`'
      'from'
      '  `mny_unidade` a'
      'order by 2')
    SQLConnection = FrmLogin.conWebMaster
    Left = 241
    Top = 232
    object QryUnidadeuni_codigo: TSmallintField
      FieldName = 'uni_codigo'
      Required = True
    end
    object QryUnidadeuni_nome: TStringField
      FieldName = 'uni_nome'
      Size = 60
    end
  end
  object DtSUnidade: TDataSource
    DataSet = QryUnidade
    Left = 273
    Top = 232
  end
  object sp_mny_saldo_dia: TSQLStoredProc
    MaxBlobSize = -1
    Params = <>
    SQLConnection = FrmLogin.conWebMaster
    Left = 560
    Top = 48
  end
  object QrySaldo: TSQLQuery
    MaxBlobSize = -1
    Params = <
      item
        DataType = ftString
        Name = 'cax_num'
        ParamType = ptInput
        Value = '1'
      end
      item
        DataType = ftString
        Name = 'cax_saldo'
        ParamType = ptInput
        Value = '0'
      end>
    SQL.Strings = (
      'call sp_mny_saldo_dia(:cax_num, :cax_saldo);')
    SQLConnection = FrmLogin.conWebMaster
    Left = 600
    Top = 48
  end
end
