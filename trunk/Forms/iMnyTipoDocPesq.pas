...
var
  FrmTipoDocPesq: TFrmTipoDocPesq;
...
procedure TFrmTipoDocPesq.FormShow(Sender: TObject);
begin
...
procedure TFrmTipoDocPesq.ActnIncExecute(Sender: TObject);
begin
  Application.MessageBox(pchar(FrmLogin.GR_Registro(ClntDtStMastertip_inc.Value)),
    'Informação', MB_OK+MB_ICONINFORMATION);
end;

procedure TFrmTipoDocPesq.ActnAltExecute(Sender: TObject);
begin
  Application.MessageBox(pchar(FrmLogin.GR_Registro(ClntDtStMastertip_alt.Value)),
    'Informação', MB_OK+MB_ICONINFORMATION);
end;

procedure TFrmTipoDocPesq.btnFecharClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmTipoDocPesq.ClntDtStMasterNewRecord(DataSet: TDataSet);
begin
  FrmLogin.GR_Refresh;
  with QryMax do
  begin
...
procedure TFrmTipoDocPesq.ClntDtStMasterBeforePost(DataSet: TDataSet);
var
  sSql: String;
begin
   FrmLogin.GR_Refresh;
   ...
   if ClntDtStMaster.State in[dsEdit] then
    begin
      ...
      begin
        sSql := SQL.Text;
        SQL.Clear;
        SQL.Add('update mny_tipo_documento a set');
  ...
  if ClntDtStMaster.State in[dsInsert] then
    begin
      ...
      begin
        sSql := SQL.Text;
        SQL.Clear;
        SQL.Add('insert into mny_tipo_documento ');
...
procedure TFrmTipoDocPesq.btnPesquisarClick(Sender: TObject);
begin
  FrmLogin.GR_Refresh;
  With QryMaster Do
    Begin
...
procedure TFrmTipoDocPesq.ClntDtStMasterBeforeDelete(DataSet: TDataSet);
var
  sSQL :String;
begin
  FrmLogin.GR_Refresh;
  with QryTransaction do
    begin
      sSql := SQL.Text;
      SQL.Clear;
      SQL.Add('delete from mny_tipo_documento ');
...
procedure TFrmTipoDocPesq.btnNovoClick(Sender: TObject);
begin
  ClntDtStMaster.Append;
end;

procedure TFrmTipoDocPesq.btnEditarClick(Sender: TObject);
begin
  ClntDtStMaster.Post;
end;
