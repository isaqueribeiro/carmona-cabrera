...
var
  FrmTipoPesPesq: TFrmTipoPesPesq;
...
procedure TFrmTipoPesPesq.FormShow(Sender: TObject);
begin
...
procedure TFrmTipoPesPesq.ActnIncExecute(Sender: TObject);
begin
  Application.MessageBox(pchar(FrmLogin.GR_Registro(ClntDtStMastertip_inc.Value)),
    'Informação', MB_OK+MB_ICONINFORMATION);
end;

procedure TFrmTipoPesPesq.ActnAltExecute(Sender: TObject);
begin
  Application.MessageBox(pchar(FrmLogin.GR_Registro(ClntDtStMastertip_alt.Value)),
    'Informação', MB_OK+MB_ICONINFORMATION);
end;

procedure TFrmTipoPesPesq.btnFecharClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmTipoPesPesq.ClntDtStMasterNewRecord(DataSet: TDataSet);
begin
  FrmLogin.GR_Refresh;
  with QryMax do
  begin
...
procedure TFrmTipoPesPesq.ClntDtStMasterBeforePost(DataSet: TDataSet);
var
  sSql: String;
begin
   FrmLogin.GR_Refresh;
   ...
   if ClntDtStMaster.State in[dsEdit] then
    begin
      ...
      begin
        sSql := SQL.Text;
        SQL.Clear;
        SQL.Add('update mny_tipo_pessoa a set');
  ...
  if ClntDtStMaster.State in[dsInsert] then
    begin
      ...
      begin
        sSql := SQL.Text;
        SQL.Clear;
        SQL.Add('insert into mny_tipo_pessoa ');
...
procedure TFrmTipoPesPesq.btnPesquisarClick(Sender: TObject);
begin
  FrmLogin.GR_Refresh;
  With QryMaster Do
    Begin
...
procedure TFrmTipoPesPesq.ClntDtStMasterBeforeDelete(DataSet: TDataSet);
var
  sSQL :String;
begin
  FrmLogin.GR_Refresh;
...
procedure TFrmTipoPesPesq.btnNovoClick(Sender: TObject);
begin
  ClntDtStMaster.Append;
end;

procedure TFrmTipoPesPesq.btnEditarClick(Sender: TObject);
begin
  ClntDtStMaster.Post;
end;
