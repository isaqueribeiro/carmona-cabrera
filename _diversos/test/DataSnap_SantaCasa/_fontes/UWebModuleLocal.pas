...
type
  TWebModuleLocal = class(TWebModule)
...
var
  WebModuleClass: TComponentClass = TWebModuleLocal;
...
uses
  UServerMethods, UServerContainer, Web.WebReq;

procedure TWebModuleLocal.ServerFunctionInvokerHTMLTag(Sender: TObject; Tag: TTag;
  const TagString: string; TagParams: TStrings; var ReplaceText: string);
begin
...
procedure TWebModuleLocal.WebModuleDefaultAction(Sender: TObject;
  Request: TWebRequest; Response: TWebResponse; var Handled: Boolean);
begin
...
procedure TWebModuleLocal.WebModuleBeforeDispatch(Sender: TObject;
  Request: TWebRequest; Response: TWebResponse; var Handled: Boolean);
begin
...
function TWebModuleLocal.AllowServerFunctionInvoker: Boolean;
begin
  Result := (Request.RemoteAddr = '127.0.0.1') or
    (Request.RemoteAddr = '0:0:0:0:0:0:0:1') or (Request.RemoteAddr = '::1');
end;

procedure TWebModuleLocal.WebFileDispatcherBeforeDispatch(Sender: TObject;
  const AFileName: string; Request: TWebRequest; Response: TWebResponse;
  var Handled: Boolean);
var
  D1, D2: TDateTime;
...
procedure TWebModuleLocal.WebModuleCreate(Sender: TObject);
begin
